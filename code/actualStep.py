...
def actual_step(desired, possible):
    ...
    values = np.zeros([3,3])-1      # initialize 3x3 matrix, where the correlation the desired step vector
    # and the unit vectors pointing in all directions (cells of the matrix) are saved
    ...
    for x in range(-1,2):
        for y in range(-1,2):
            if possible[x+1, y+1]:      # if field accessible calculate correlation
                # between direction vector for that field and desired vector
