...
ant_start_position = [799, 50]       # starting position (possible values: ints between 1 and 1598)
global_vector_start = np.array([0, 750])      # global vector at the starting position
...
end_position = [1, 1]     # where the channel ends
...
# channel exit
channel_exit_influence = 0
exit_direction = [0, -1]     # enter normed vector here # direction in which the ant leaves the channel
exit_push_strength = 5

# object
object_positions = [[750, 150], [850, 250], [750, 350], [850, 450], [750, 550], [850, 650]]     # objects help the ant for orientation with local vector
...
# familiar path
start_of_path = [799, 100]       # define a path direction but only influences the local vector if objects are near
